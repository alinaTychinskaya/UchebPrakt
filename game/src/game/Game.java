...
public class Game extends JFrame { //класс Game наследует от класса JFrame
    ...
    private static float drop_left = 130; //координата точки угла падающего объекта
    private static float drop_top = -50; //координата точки угла падающего объекта
    ...
    public static void main(String[] args) throws IOException { //обработка исключения
        ...
        background = ImageIO.read(Game.class.getResourceAsStream("one.jpg")); //загрузка картинки
        game_over = ImageIO.read(Game.class.getResourceAsStream("gameover.jpg")); //загрузка картинки
        drop = ImageIO.read(Game.class.getResourceAsStream("we.jpeg")); //загрузка картинки
        ...
        game_window.setLocation(500, 50); //точка появления окна программы
        game_window.setSize(1000, 600); //размер окна
 ...
 */
        ...
        if(drop_top > game_window.getHeight()) g.drawImage(game_over, 80, 50, null); //отрисовка картинки
    }
